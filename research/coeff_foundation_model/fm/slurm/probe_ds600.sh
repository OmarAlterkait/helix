...
#SBATCH --partition=ampere --account=mli:default --qos=preemptable --gpus=1 --cpus-per-task=16 --mem=96G
#SBATCH --time=02:00:00 --requeue
...
P(){ singularity exec --nv -B /sdf $IMG python3 probe_3d_mlp.py --out probe_ds600.jsonl --serial 1 --rope_split 0 --events 30000-30299 "$@"; }
for D in 20k 80k; do
  for CK in ckpt_dscale600_${D}_snap*.pt ckpt_dscale600_${D}.pt; do
    [ -f "$CK" ] || continue
    S=$(echo "$CK" | grep -oE 'snap[0-9]+' || echo final)
    grep -q "\"ds${D}_${S}\"" probe_ds600.jsonl 2>/dev/null && { echo "skip $D $S (done)"; continue; }
    echo "### probing $D $S ###"; P --ckpt "$CK" --tag ds${D}_${S#snap}
  done
done
echo DONE
