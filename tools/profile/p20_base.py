...
from common import pack, synth_from
print("\n== best stack, validated on 5 real events ==", flush=True)

def ship_step(i):
    with torch.autocast("cuda", torch.bfloat16):
        f = model.forward_feat(evs_d[i], masks[i])
        return head_base(model, f, evs_d[i], masks[i])

def best_step(i, bf16=False, compiled=True):
    with torch.autocast("cuda", torch.bfloat16):
        fm_, mi = ffeat_masked(model, evs_d[i], masks[i], stream_bf16=bf16)
        return head_masked(model, fm_, evs_d[i], masks[i], mi)
...
for i in range(5):
    with torch.no_grad():
        l0 = sum(ship_step(i)).item()
        l1 = sum(best_step(i)).item()
        l2 = sum(best_step(i, bf16=True)).item()
    R["validate"][f"event{i}"] = dict(n_cells=int(evs_d[i]["n_cells"]), shipped=l0,
                                      best=l1, d=l1 - l0, rel=(l1 - l0) / l0,
                                      best_bf16=l2, d_bf16=l2 - l0)
    print(f"  ev{i} N={int(evs_d[i]['n_cells']):6d}  loss {l0:.6f} -> {l1:.6f} "
          f"(d={l1-l0:+.2e}, rel {100*(l1-l0)/l0:+.4f}%)   bf16-stream d={l2-l0:+.2e}", flush=True)
...
print("\n== throughput and memory ==", flush=True)
R["final"] = {}
for tag, fn in (("A shipped", ship_step),
                ("H best (bit-faithful)", lambda i: best_step(i, False)),
                ("I best + bf16 stream", lambda i: best_step(i, True))):
    st = mk(fn)
    t = timeit(st, warmup=3, iters=A.iters)
    with peak_mem() as pm:
        st()
    R["final"][tag] = dict(ms=t["ms_med"], peak_MiB=pm["peak_alloc_MiB"],
                           ev_per_s=1000 / t["ms_med"])
    print(f"  {tag:24s} {t['ms_med']:8.2f} ms  peak {pm['peak_alloc_MiB']:8.0f} MiB  "
          f"{1000/t['ms_med']:.2f} ev/s", flush=True)
    opt.zero_grad(set_to_none=True); gc.collect(); torch.cuda.empty_cache()
a = R["final"]["A shipped"]
for k, v in R["final"].items():
    v["speedup"] = a["ms"] / v["ms"]; v["mem"] = v["peak_MiB"] / a["peak_MiB"]
    print(f"  {k:24s} x{v['speedup']:.3f}  mem {v['mem']:.2f}x")

print("\n== busy fraction on the fast path ==", flush=True)
import threading
def busy(tag, st, secs=8):
    ...
    ka = pr.key_averages()
    ker = sum(e.self_device_time_total for e in ka) / 1e3 / A.iters
    nk = sum(e.count for e in ka if e.self_device_time_total > 0) / A.iters
    R.setdefault("busy", {})[tag] = dict(ms=wall / n * 1e3, util=float(np.mean(sm)),
                                         busy_frac=ker / pw, kernels=nk)
    print(f"  {tag:24s} {wall/n*1e3:8.1f} ms  util {np.mean(sm):5.1f}%  "
          f"kernel/wall {100*ker/pw:5.1f}%  kernels {nk:.0f}", flush=True)
    opt.zero_grad(set_to_none=True); gc.collect(); torch.cuda.empty_cache()

busy("A shipped", mk(ship_step))
busy("H best", mk(lambda i: best_step(i, False)))

print("\n== token ceiling on the fast path ==", flush=True)
src = {k: v for k, v in evs[0].items() if torch.is_tensor(v)}
for tag, fn in (("A shipped", None), ("H best", True)):
    lo, hi, best = 16000, 800000, 0
    while hi - lo > 16000:
        mid = (lo + hi) // 2
        try:
            Bs = to_device(synth_from(src, mid), dev); Bs["n_cells"] = mid
            g2 = torch.Generator(device=dev); g2.manual_seed(0)
            ms = model.make_mask(Bs, mode="random", gen=g2)
            opt.zero_grad(set_to_none=True)
            with torch.autocast("cuda", torch.bfloat16):
                if fn is None:
                    f = model.forward_feat(Bs, ms); b_, v_ = head_base(model, f, Bs, ms)
                else:
                    fm_, mi = ffeat_masked(model, Bs, ms); b_, v_ = head_masked(model, fm_, Bs, ms, mi)
            (b_ + v_).backward()
            torch.cuda.synchronize()
            best = mid; lo = mid
            ...
            Bs = ms = None; opt.zero_grad(set_to_none=True)
            gc.collect(); torch.cuda.empty_cache()
    R.setdefault("ceiling", {})[tag] = dict(max_cells=best, events=best / 32500.0)
    print(f"  {tag:12s} max_cells={best:7d}  ~{best/32500.0:.1f} events", flush=True)
