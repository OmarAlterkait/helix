...
# ============================ (c) a fused rotary kernel that already exists
print("\n== (c) flash_attn's fused rotary vs the hand-fused form ==", flush=True)
FA_OK = False
try:
    from flash_attn.layers.rotary import apply_rotary_emb as fa_rotary
    FA_OK = True
except Exception as e:
    print("  flash_attn rotary unavailable:", e)

def rope_fa(x, cos, sin):
    """flash_attn's fused rotary. helix rotates INTERLEAVED pairs (v[...,1::2],
    v[...,0::2]), which is `interleaved=True`; passing cos/sin that cover all
    hd/2 pairs (time frequencies for the first hd/4, wire for the rest) does
    both halves in the one kernel."""
    return fa_rotary(x[None], cos, sin, interleaved=True).squeeze(0)

R["rope_fa"] = {}
if FA_OK:
    for tag, T in (("encoder", int((~m).sum())), ("decoder q", int(m.sum()))):
        at = rope_angles(torch.rand(T, device=dev) * 4000, HD, *model.lam_t)
        aw = rope_angles(torch.rand(T, device=dev) * 2000, HD, *model.lam_w)
        for dt in (torch.float32, torch.bfloat16):
            x = torch.randn(T, model.heads, HD, device=dev, dtype=dt)
            c2, s2 = rope_pairs(at, aw, dt)
            cos = torch.cat([torch.cos(at), torch.cos(aw)], -1).to(dt)
            sin = torch.cat([torch.sin(at), torch.sin(aw)], -1).to(dt)
            ref_ = apply_rope(x, at, aw)
            try:
                got = rope_fa(x, cos, sin)
                dv = float((ref_.float() - got.float()).abs().max())
                rel = dv / float(ref_.float().abs().max())
                t = timeit(lambda: rope_fa(x, cos, sin), warmup=5, iters=30)["ms_med"]
            except Exception as e:
                dv = rel = float("nan"); t = float("nan")
                print("    fa failed:", e)
            tf = timeit(lambda: rope_fused(x, c2, s2), warmup=5, iters=30)["ms_med"]
            ideal = 2 * x.numel() * x.element_size() / (1285 * 2**30) * 1e3
            k = f"{tag} {str(dt).split('.')[-1]}"
            R["rope_fa"][k] = dict(fa_ms=t, fused_ms=tf, ideal_ms=ideal,
                                   max_abs_dev=dv, max_rel_dev=rel,
                                   fa_pct_of_roofline=100 * ideal / t if t == t else None)
            print(f"  {k:26s} flash_attn {t:6.3f}  hand-fused {tf:6.3f}  ideal {ideal:6.3f} ms"
                  f"  ({100*ideal/t:4.1f}% of roofline)  max|d|={dv:.2e} rel={rel:.2e}", flush=True)
            del x, c2, s2, cos, sin, ref_
            gc.collect(); torch.cuda.empty_cache()
