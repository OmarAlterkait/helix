...
# ===================== how far does width go, and what unlocks the next step
...
def ck_self(*a, **k):
    return checkpoint(_sp, *a, use_reentrant=False, **k)


def ck_cross(*a, **k):
    return checkpoint(_cp, *a, use_reentrant=False, **k)


def run(ov, ckpt, iters=4):
    global _self_perm, _cross_perm
    mo = build(ov, device=dev)
    p = n_params(mo)
    o = torch.optim.AdamW(mo.param_groups(0.0, weight_decay=0.05), betas=(0.9, 0.95))
    g = torch.Generator(device=dev); g.manual_seed(0)
    mk = mo.make_mask(Bs, mode="random", gen=g)
    if ckpt:
        globals()["_self_perm"], globals()["_cross_perm"] = ck_self, ck_cross
    try:
        def st():
            ...
            o.step()
        st(); st()
        state = torch.cuda.memory_allocated() / 2**20
        t = timeit(st, warmup=1, iters=iters)
        with peak_mem() as pm:
            st()
...
CFG = [
    ("d=1024 enc12", dict(d=1024, heads=16)),
    ("d=1536 enc12", dict(d=1536, heads=24)),
    ("d=2048 enc12", dict(d=2048, heads=32)),
    ("d=2560 enc12", dict(d=2560, heads=40)),
    ("d=3072 enc12", dict(d=3072, heads=48)),
    ("d=1024 enc24", dict(d=1024, heads=16, blocks=24)),
    ("d=1536 enc24", dict(d=1536, heads=24, blocks=24)),
    ("d=2048 enc24", dict(d=2048, heads=32, blocks=24)),
]
BASE = dict(d=512, blocks=12, dec_blocks=4, heads=8)
R["big"] = {}
print(f"{'config':16s} {'ckpt':>5s} {'params':>9s} {'ms':>8s} {'peak':>9s} "
      f"{'state':>8s} {'activ':>9s}", flush=True)
for tag, ov in CFG:
    for ckpt in (False, True):
        try:
            r = run({**BASE, **ov}, ckpt)
            R["big"][f"{tag} ckpt={int(ckpt)}"] = r
            print(f"  {tag:16s} {str(ckpt):>5s} {r['params_M']:8.1f}M {r['ms']:8.1f} "
                  f"{r['peak_MiB']:9.0f} {r['state_MiB']:8.0f} {r['act_MiB']:9.0f}", flush=True)
        except torch.cuda.OutOfMemoryError:
            R["big"][f"{tag} ckpt={int(ckpt)}"] = dict(oom=True)
            print(f"  {tag:16s} {str(ckpt):>5s} OOM", flush=True)
            gc.collect(); torch.cuda.empty_cache()
        except Exception as e:
            R["big"][f"{tag} ckpt={int(ckpt)}"] = dict(error=repr(e)[:160])
            print(f"  {tag:16s} {str(ckpt):>5s} ERR {e}", flush=True)
            gc.collect(); torch.cuda.empty_cache()

# ----------------------------------- what optimizer-state sharding would free
print("\n== optimizer state, and what ZeRO-2 across the DDP ranks would free ==",
      flush=True)
R["zero"] = {}
for tag, r in R["big"].items():
    if "params_M" not in r or "ckpt=0" not in tag:
        continue
    Pn = r["params_M"] * 1e6
    par, grad, adam = Pn*4/2**20, Pn*4/2**20, 2*Pn*4/2**20
    for w in (4, 8, 16):
        shard = par + grad/w + adam/w
        R["zero"][f"{tag} world={w}"] = dict(
            full_MiB=par+grad+adam, sharded_MiB=shard,
            new_peak_MiB=r["peak_MiB"] - (par+grad+adam) + shard)
    w = 8
    s = R["zero"][f"{tag} world=8"]
    print(f"  {tag:22s} state {par+grad+adam:7.0f} -> {s['sharded_MiB']:7.0f} MiB at 8 ranks"
          f"   peak {r['peak_MiB']:7.0f} -> {s['new_peak_MiB']:7.0f} MiB", flush=True)
