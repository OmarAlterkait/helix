...
# =================================================== what does dynamo refuse?
import torch._dynamo as dyn

def head_masked_wrap(fm_, occt, validm, tgtm, bandm, W, bv, edges, rmult):
    pass  # placeholder, real one below


def explain(tag, fn, *args, **kw):
    try:
        e = dyn.explain(fn)(*args, **kw)
        reasons = []
        for b in getattr(e, "break_reasons", []):
            loc = ""
            try:
                loc = " | ".join(f"{f.filename.split('/')[-1]}:{f.lineno} {f.name}"
                                 for f in (b.user_stack or [])[-2:])
            except Exception:
                pass
            reasons.append(f"{getattr(b, 'reason', '?')}  @ {loc}")
        R.setdefault("explain", {})[tag] = dict(
            graphs=getattr(e, "graph_count", None),
            breaks=getattr(e, "graph_break_count", None),
            ops=getattr(e, "op_count", None),
            reasons=reasons)
        print(f"  {tag:26s} graphs={getattr(e,'graph_count','?')} "
              f"breaks={getattr(e,'graph_break_count','?')} ops={getattr(e,'op_count','?')}")
        for r in reasons[:12]:
            print(f"       - {r[:150]}")
    except Exception as ex:
        R.setdefault("explain", {})[tag] = dict(error=repr(ex))
        print(f"  {tag:26s} explain failed: {ex}")
    finally:
        dyn.reset()


print("== dynamo: where the graph breaks ==", flush=True)
with torch.autocast("cuda", torch.bfloat16):
    explain("model.forward (shipped)", lambda: model(B, tok_mask=m))
    explain("forward_feat (shipped)", lambda: model.forward_feat(B, m))
    explain("ffeat_perm (fast)", lambda: ffeat_perm(model, B, m, stream_bf16=False, perm_dec=True))
    explain("ffeat_masked (fast)", lambda: ffeat_masked(model, B, m))
    with torch.no_grad():
        f_ = ffeat_perm(model, B, m, stream_bf16=False, perm_dec=True)
        fm_, mi_ = ffeat_masked(model, B, m)
    explain("head_bmm", lambda: head_bmm(model, f_, B, m))
    explain("head_masked", lambda: head_masked(model, fm_, B, m, mi_))
    explain("losses_cat (dense)", lambda: losses_cat(
        model.occ_head(f_), model.val_head(f_).view(-1, model.n_slot, model.n_bins),
        B, m, model.bin_edges, vis_w=0.0))
    xp_ = torch.randn(8192, model.d, device=dev, dtype=torch.float32)
    c_, s_ = rope_pairs(rope_angles(torch.rand(8192, device=dev) * 4000, HD, *model.lam_t),
                        rope_angles(torch.rand(8192, device=dev) * 2000, HD, *model.lam_w),
                        xp_.dtype)
    explain("_self_perm (one block)", lambda: _self_perm(model.enc[0], xp_, c_, s_, 8, 1024))
    explain("blk.mlp", lambda: model.enc[0].mlp(model.enc[0].n2(xp_)))
    explain("model._sched", lambda: model._sched(B["plane_id"], B["t_phys"], B["wire_pos"]))
    explain("model.make_mask", lambda: model.make_mask(B, mode="random"))

# =============================== compiling progressively more of the fast path
print("\n== compile coverage on the fast path ==", flush=True)
...
def restore():
    ...
    globals()["_self_perm"] = _sp
    globals()["_cross_perm"] = _cp
    globals()["head_masked"] = _hm
    dyn.reset(); gc.collect(); torch.cuda.empty_cache()


R["coverage"] = {}
def cover(tag, setup, warm=6):
    try:
        restore(); setup()
        for _ in range(warm):
            step_fast()
        torch.cuda.synchronize()
        t = timeit(step_fast, warmup=3, iters=A.iters)
        with peak_mem() as pm:
            step_fast()
        R["coverage"][tag] = dict(ms=t["ms_med"], peak_MiB=pm["peak_alloc_MiB"])
        print(f"  {tag:34s} {t['ms_med']:8.2f} ms  peak {pm['peak_alloc_MiB']:8.0f} MiB", flush=True)
    except Exception as e:
        R["coverage"][tag] = dict(error=repr(e), tb=traceback.format_exc()[-500:])
        print(f"  {tag:34s} FAILED {e}")
    finally:
        opt.zero_grad(set_to_none=True); restore()

C = lambda f, **kw: torch.compile(f, dynamic=True, **kw)
cover("eager fast path", lambda: None, warm=3)
cover("+ compile rope", lambda: globals().__setitem__("rope_fused", C(_rf)))
cover("+ compile mlp", lambda: [setattr(b, "mlp", C(b.mlp)) for b in list(model.enc) + list(model.dec)])
cover("+ compile enc block", lambda: globals().__setitem__("_self_perm", C(_sp)))
cover("+ compile enc+dec block", lambda: (globals().__setitem__("_self_perm", C(_sp)),
                                          globals().__setitem__("_cross_perm", C(_cp))))
cover("+ compile head/loss", lambda: globals().__setitem__("head_masked", C(_hm)))
cover("+ compile blocks + rope", lambda: (globals().__setitem__("_self_perm", C(_sp)),
                                          globals().__setitem__("_cross_perm", C(_cp)),
                                          globals().__setitem__("rope_fused", C(_rf))))
cover("+ blocks + rope + head", lambda: (globals().__setitem__("_self_perm", C(_sp)),
                                         globals().__setitem__("_cross_perm", C(_cp)),
                                         globals().__setitem__("rope_fused", C(_rf)),
                                         globals().__setitem__("head_masked", C(_hm))))
torch._dynamo.config.capture_scalar_outputs = True
cover("blocks+rope+head, scalar capture",
      lambda: (globals().__setitem__("_self_perm", C(_sp)),
               globals().__setitem__("_cross_perm", C(_cp)),
               globals().__setitem__("rope_fused", C(_rf)),
               globals().__setitem__("head_masked", C(_hm))))
torch._dynamo.config.capture_scalar_outputs = False

# ------------------ recompiles under real shape churn (5 different events)
print("\n== recompiles across 5 real events ==", flush=True)
restore()
globals()["_self_perm"] = C(_sp); globals()["_cross_perm"] = C(_cp)
globals()["rope_fused"] = C(_rf)
def rr():
    for i in range(5):
        opt.zero_grad(set_to_none=True)
        with torch.autocast("cuda", torch.bfloat16):
            f, mi = ffeat_masked(model, evs_d[i], masks[i])
            b_, v_ = head_masked(model, f, evs_d[i], masks[i], mi)
        ((b_ + v_) / 5).backward()
    opt.step()
...
R["churn"] = dict(compiled_ms_per_event=t["ms_med"] / 5, warmup_s=warm,
                  frame_count=len(dyn.utils.counters.get("frames", {})) if hasattr(dyn, "utils") else None,
                  counters={k: dict(v) for k, v in dyn.utils.counters.items()} if hasattr(dyn, "utils") else None)
print(f"  compiled, 5 events round-robin: {t['ms_med']/5:.2f} ms/event (warmup {warm:.1f}s)")
restore()
def rr_e():
    for i in range(5):
        opt.zero_grad(set_to_none=True)
        ...
        ((b_ + v_) / 5).backward()
    opt.step()
t = timeit(rr_e, warmup=2, iters=4)
R["churn"]["eager_ms_per_event"] = t["ms_med"] / 5
print(f"  eager,    5 events round-robin: {t['ms_med']/5:.2f} ms/event")

emit("p21_compile_coverage", R)
